...
export function CartProvider({ children }) {
    ...
    useEffect(() => {
        // 1. Higiene básica (borrar guest cart si aplica)
        if (user && user.email) {
            localStorage.removeItem('tienda_cart_guest');
        }

        // 2. Intentar recuperar del Registry (Nube simulada)
        if (user && user.email) {
            const savedCart = getUserCart(user.email);
            if (savedCart && savedCart.length > 0) {
                // Si hay datos en el registry, tienen prioridad
                setCart(savedCart);
                return;
            }
        }

        // 3. Fallback a LocalStorage normal
        const key = getStorageKey();
        const stored = localStorage.getItem(key);
        try {
            if (stored) {
                ...
                if (parsed.owner === currentOwner) {
                    ...
                    // Si el dueño no coincide, ignoramos
                    setCart([]);
                }
            } else {
                // Si no hay nada guardado y no somos usuarios que recuperan del registry
                if (!user) setCart([]);
            }
        } catch (error) {
            setCart([]);
        }
    }, [user, getStorageKey, getUserCart]);
    ...
    useEffect(() => {
        ...
        // Nuevo: Sincronizar "Nube" (Registry) si es usuario
        if (user && user.email) {
            updateUserCart(user.email, cart);
        }
    }, [cart, user, getStorageKey, updateUserCart]);
