...
export default function Login() {
    ...
    const { login, validateCredentials, checkUserExists } = useUser();
    ...
    const handleGoogleLogin = () => {
        const googleUser = {
            ...
            password: 'google_auth_token' // Dummy password for internal logic
        };

        // If user exists or not, we log them in immediately for UX simpicity in this demo
        // Ideally we would check if they need to complete profile
        login(googleUser);
        addToast(`¡Bienvenido, ${googleUser.name}!`);
        navigate('/');
    };
    ...
    const handleLoginSubmit = (e) => {
        ...
        // 1. Try to login
        const user = validateCredentials(loginData.email, loginData.password);
        ...
        if (user) {
            ...
            if (checkUserExists(loginData.email)) {
                addToast('Contraseña incorrecta', 'error');
            } else {
                addToast('Usuario no encontrado. ¿Deseas registrarte?', 'info');
                // Optional: Auto switch to register?
    ...
    const handleRegisterStep1 = (e) => {
        ...
        if (checkUserExists(registerData.email)) {
            addToast('Este email ya está registrado. Por favor inicia sesión.', 'error');
            setMode('login');
    ...
    const handleAddressSubmit = (addressData) => {
        ...
        login(registerData, addressData);
